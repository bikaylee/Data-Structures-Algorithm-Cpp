...
template<class ItemType>
int LinkedList<ItemType>::replaceItem(const ItemType& oldValue, const ItemType& newValue)
{
    //Find the node pointing at oldValue, replace item by new value

    int occurrence = 0;
    Node<ItemType>* curPtr = headPtr;
    for(int pos = 0; pos < itemCount; pos++)
    {
        if(curPtr->getItem() == oldValue)
        {
